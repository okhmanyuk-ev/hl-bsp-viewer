...
private:
	...
	using Vertex = Renderer::Vertex::PositionTextureNormal;
	...
	std::map<TexId, std::shared_ptr<Renderer::Texture>> mTextures;
	
	bool mLightAnimation = false;
	glm::vec3 mLightPosition;

	std::shared_ptr<Renderer::RenderTarget> mSceneTarget = std::make_shared<Renderer::RenderTarget>(PLATFORM->getWidth(), PLATFORM->getHeight());
